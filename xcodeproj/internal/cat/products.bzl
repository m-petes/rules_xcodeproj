...
load("//xcodeproj/internal:files.bzl", "join_paths_ignoring_empty")
...
def _collect_product(
    ...
    Returns:
        ...
        *   `xcode_product`: A `struct` with the following fields:
            ...
            *   `original_path`: `path` if the product isn't codesigned,
                otherwise it's the path to the uncodesigned binary.
    ...
    return struct(
        ...
        xcode_product = struct(
            basename = basename,
            # FIXME: Replace with `original_basename`
