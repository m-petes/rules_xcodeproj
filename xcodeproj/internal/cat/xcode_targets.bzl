...
def _from_resource_bundle(bundle, *, bundle_id):
    return struct(
        ...
        direct_dependencies = EMPTY_DEPSET,
        has_c_params = False,
        ...
        module_name_attribute = EMPTY_STRING,
        outputs = struct(
        ...
        product = _product_from_resource_bundle(bundle),
...
def _product_from_resource_bundle(bundle):
    basename = bundle.name + ".bundle"

    return struct(
        basename = basename,
        original_path = "{}/{}".format(bundle.package_bin_dir, basename),
