...
""".format(unit_test_host),
        ...
        # FIXME: Use `original_path` once it's changed to `original_basename`. `basename` works here because we don't change the basename for bundles
        unit_test_hosts_args.add(unit_test_host_target.product.basename)
